...
;(function (Math) {
    ...
    // 可以指定小数位的四舍五入算法
    Number.prototype.round2 = function (digits) {

        return new Decimal(this).round(digits).value;
    }
    ...
    // 此处统一处理为四舍五入
    Number.prototype.toFixed2 = (1.115).toFixed(2) !== '1.11' ? (0).toFixed : function (digits) {
...
flyingon.renderer('Grid', function (base) {
    ...
    function change_focus(grid, cell) {
        ...
        if (column)
        {
            if (column.focused() && (focus = grid.__focus_cell) !== cell)
            {
    ...
    this.__show_body = function (grid, rows, height) {
        ...
        if (any = grid.__show_row)
        {
            grid.view_scroll.scrollTop = any.__show_index * rowHeight;
        }
    ...
    this.__set_current = function (row, patch) {
        ...
        if (any !== row)
        {
            ...
            if (patch && this.view)
            {
                if (any && any.view)
                {
                    any.renderer.patch(any, 'current', false);
                }

                if (row && row.view)
                {
                    row.renderer.patch(row, 'current', true);
                }
                else if (!this.__click_move)
                {
                    this.__show_row = row;
                }
            }
